...
IpCacheSharedPtr IpCache::newIpCache(Server::Configuration::ServerFactoryContext& context,
                                     ...
                                     std::chrono::milliseconds cache_entry_lifetime) {
  auto ipcache = context.singletonManager().getTyped<Cilium::IpCache>(
      SINGLETON_MANAGER_REGISTERED_NAME(cilium_ipcache), [&path, &context, &cache_entry_lifetime] {
        auto ipcache = std::make_shared<Cilium::IpCache>(context, path, cache_entry_lifetime);
        if (!ipcache->open()) {
          ipcache.reset();
        }
        return ipcache;
...
IpCache::IpCache(Server::Configuration::ServerFactoryContext& context, const std::string& path,
    ...
    : Bpf(BPF_MAP_TYPE_LPM_TRIE, sizeof(struct IpCacheKey), sizeof(SecLabelType),
      ...
      cache_entry_lifetime_(cache_entry_lifetime), cache_gc_interval_(10 * cache_entry_lifetime_),
      time_source_(context.timeSource()), path_(path) {
  // Timer for cache GC
...
uint32_t IpCache::resolve(const Network::Address::Ip* ip) {
  ...
  // local cache lookup
  {
    ...
    if (it != cache_.end()) {
      ...
      if (age < std::chrono::milliseconds(1)) {
        // use cached value
        ENVOY_LOG(debug, "cilium.ipcache: {} has cached ID {}", ip->addressAsString(),
                  it->second.sec_label);
        return it->second.sec_label;
      }
      // cached value too old, remove it
      ENVOY_LOG(trace, "cilium.ipcache: removing too old cached value for {}",
                ip->addressAsString());
      cache_.erase(it);
    }
  }
  ...
  if (lookup(&key, &value)) {
    ...
    // cache result
    Thread::LockGuard guard(mutex_);
    cache_.insert_or_assign(key, CachedEndpointInfo{value.sec_label, time_source_.monotonicTime()});
