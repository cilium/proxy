...
#define CILIUM_ENVOY_WRAPPER_FD 3

// Communication with the wrapper process is performed with a simple message protocol over a Unix
...
// WrapperProtocol implements the logic for communicating with the wrapper process.
class WrapperProtocol {
public:
  WrapperProtocol(int fd);
  ~WrapperProtocol();
