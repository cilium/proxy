...
public:
  SocketMarkOption(uint32_t mark, uint32_t identity, bool enable_reuse_port,
  ...
  bool setOption(Network::Socket& socket,
    ...
    if (source_address || identity_ == 0) {
      ...
      if (enable_reuse_port_) {
        ...
        ENVOY_LOG(trace, "Skip setting socket ({}) option SO_REUSEPORT",
                  socket.ioHandle().fdDoNotUse());
      }
    }
...
public:
  SocketOption(PolicyInstanceConstSharedPtr policy, uint32_t mark, uint32_t ingress_source_identity,
      ...
      : SocketMarkOption(mark, source_identity, enable_reuse_port, original_source_address,
