...
public:
 SocketOption(std::shared_ptr<const PolicyInstance> policy, uint32_t source_identity, uint32_t destination_identity, bool ingress, uint16_t port, std::string&& pod_ip, Network::Address::InstanceConstSharedPtr src_address)
   : SocketMarkOption(source_identity, ingress, src_address), policy_(policy), destination_identity_(destination_identity), port_(port), pod_ip_(std::move(pod_ip)) {
