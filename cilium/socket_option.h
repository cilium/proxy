...
public:
  ...
  bool setOption(Network::Socket& socket,
    ...
    auto status = setSockOpt(socket.ioHandle().fdDoNotUse(), SOL_SOCKET, SO_MARK, &mark_, sizeof(mark_));
    ...
    if (!ipVersion.has_value()) {
      ENVOY_LOG(critical,
		"Socket address family is not available, can not set socket options");
      return false;
    }
    ...
    if (source_address || identity_ == 0) {
      ...
      if (*ipVersion == Network::Address::IpVersion::v4) {
	auto status = setSockOpt(socket.ioHandle().fdDoNotUse(), SOL_IP, IP_TRANSPARENT, &one, sizeof(one));
	if (status.return_value_ < 0) {
	  if (status.errno_ == EPERM) {
	    // Do not assert out in this case so that we can run tests without
	    // CAP_NET_ADMIN.
	...
	auto status = setSockOpt(socket.ioHandle().fdDoNotUse(), SOL_IPV6, IPV6_TRANSPARENT, &one, sizeof(one));
	if (status.return_value_ < 0) {
	  if (status.errno_ == EPERM) {
	    // Do not assert out in this case so that we can run tests without
	    // CAP_NET_ADMIN.
