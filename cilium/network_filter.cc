...
Network::FilterStatus Instance::onNewConnection() {
  ...
  callbacks_->addUpstreamCallback([this](
    ...
    Network::Address::InstanceConstSharedPtr dst_address = option->isL7LB() ? host->address() : stream_info.downstreamAddressProvider().localAddress();
...
Network::FilterStatus Instance::onData(Buffer::Instance& data,
                                       bool end_stream) {
  ENVOY_LOG(trace, "cilium.network: onData");
