...
public:
  GrpcSubscriptionImpl(Config::GrpcMuxSharedPtr grpc_mux, Config::SubscriptionCallbacks& callbacks,
                       ...
                       bool is_aggregated)
    : Config::GrpcSubscriptionImpl(grpc_mux, callbacks, resource_decoder, stats, type_url,
                                   dispatcher, init_fetch_timeout, is_aggregated),
...
subscribe(const std::string& type_url, const LocalInfo::LocalInfo& local_info,
	  ...
	  Runtime::RandomGenerator& random, Envoy::Config::OpaqueResourceDecoder& resource_decoder,
	  Stats::Scope &scope, Envoy::Config::SubscriptionCallbacks& callbacks);
