...
uint64_t get_capabilities(unsigned int kind) {
...
WrapperProtocol::~WrapperProtocol() {
  close();
}

WrapperProtocol::WrapperProtocol(int fd) : fd_(fd) {}

void WrapperProtocol::close() {
  if (fd_ != -1) {
    ::close(fd_);
  }
  fd_ = -1;
...
ssize_t WrapperProtocol::send_fd_msg(const void *header, ssize_t headerlen, const void *data, ssize_t datalen, int fd) {
  struct iovec  iov[2];
...
ssize_t WrapperProtocol::recv_fd_msg(const void *header, ssize_t headersize, const void *data, ssize_t datasize, int *fd) {
  struct iovec  iov[2];
