...
size_t maskData(uint8_t* buf, size_t n_bytes, uint8_t mask[4], size_t mask_offset = 0) {
  for (size_t i = 0; i < n_bytes; i++) {
    buf[i] ^= mask[mask_offset];
...
void Codec::Encoder::encode(Buffer::Instance& data, uint8_t opcode) {
  ...
  if (config->client_) {
    ...
    uint8_t mask[4];
    uint32_t mask_word = config->random_.random();
    for (unsigned char& i : mask) {
      i = static_cast<uint8_t>(mask_word);
      mask_word >>= 8;
      frame_header[frame_header_length++] = i;
    }

    // mask data in place
    size_t mask_offset = 0;
    for (const Buffer::RawSlice& slice : data.getRawSlices()) {
      mask_offset = maskData(static_cast<uint8_t*>(slice.mem_), slice.len_, mask, mask_offset);
    }
  }
