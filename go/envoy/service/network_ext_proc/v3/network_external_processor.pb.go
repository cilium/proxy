...
func (c *networkExternalProcessorClient) Process(ctx context.Context, opts ...grpc.CallOption) (NetworkExternalProcessor_ProcessClient, error) {
	stream, err := c.cc.NewStream(ctx, &_NetworkExternalProcessor_serviceDesc.Streams[0], "/envoy.service.network_ext_proc.v3.NetworkExternalProcessor/Process", opts...)
	if err != nil {
		return nil, err
	}
...
// UnimplementedNetworkExternalProcessorServer can be embedded to have forward compatible implementations.
...
func RegisterNetworkExternalProcessorServer(s *grpc.Server, srv NetworkExternalProcessorServer) {
	s.RegisterService(&_NetworkExternalProcessor_serviceDesc, srv)
}
...
var _NetworkExternalProcessor_serviceDesc = grpc.ServiceDesc{
