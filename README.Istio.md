...
cilium/istio `inject-cilium-filters-1.5.4` branch, which is based on
Istio's release 1.5.4 branch. Make sure that when you build the Istio
...
Istio.
    ...
    cd istio
    git checkout inject-cilium-filters-1.5.4
    docker build -t cilium/istio_pilot:1.5.4 .
...
## Build Cilium's sidecar proxy Docker images
    ...
    cd proxy
    git checkout istio-1.5.4
    make docker-istio-proxy

## Push the Docker images to Docker Hub

    docker login -u ...
    docker image push cilium/istio_pilot:1.5.4
    docker image push cilium/istio_proxy:1.5.4
