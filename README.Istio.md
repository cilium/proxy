...
cilium/istio `inject-cilium-filters-1.9.6` branch, which is based on
Istio release 1.9.6. Make sure that when you build the Istio
...
Istio.
    ...
    cd istio
    git checkout inject-cilium-filters-1.9.6
    docker build -t cilium/istio_pilot:1.9.6 .
...
## Build Cilium's sidecar proxy Docker images
    ...
    cd proxy
    git checkout istio-1.9.6
    make docker-istio-proxy

## Push the Docker images to Docker Hub

    docker login -u ...
    docker image push cilium/istio_pilot:1.9.6
    docker image push cilium/istio_proxy:1.9.6
